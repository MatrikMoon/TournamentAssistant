    ...
    public class QualifierModule : InteractionModuleBase
    {
        ...
        public async Task CreateEventAsync(string name, string hostAddress, ITextChannel scoreChannel = null, string settings = null)
        {
            ...
            else
            {
                ...
                var host = server.GetServers().FirstOrDefault(x => $"{x.Address}:{x.Port}" == hostAddress);
        ...
        public async Task SetScoreChannelAsync(ITextChannel channel, string eventId)
        {
            ...
            else
            {
                ...
                var knownPairs = await HostScraper.ScrapeHosts(server.GetServers().ToArray(), $"{server.ServerSelf.Address}:{server.ServerSelf.Port}", 0);
        ...
        public async Task ListEventsAsync()
        {
            ...
            else
            {
                ...
                var knownEvents = (await HostScraper.ScrapeHosts(server.GetServers().ToArray(), $"{server.ServerSelf.Address}:{server.ServerSelf.Port}", 0)).Select(x => x.Value).Where(x => x.Events != null).SelectMany(x => x.Events);
        ...
        public async Task ListHostsAsync()
        {
            ...
            else
            {
                ...
                foreach (var host in server.GetServers())
                {
