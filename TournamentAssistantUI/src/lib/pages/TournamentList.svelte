<script lang="ts">
  ...
  $: tournamentsWithImagesAsUrls = tournaments.map((x) => {
    ...
    if ((byteArray?.length ?? 0) > 1) {
      ...
      if (!(x.tournament?.settings?.tournamentImage instanceof Uint8Array)) {
...
<List twoLine avatarList singleSelection>
  {#each tournamentsWithImagesAsUrls as item}
    <Item
      on:SMUI:action={() => {
        onTournamentSelected(item.tournament.guid, item.address, item.port);
        console.log(
    ...
    >
      <Graphic
        style="background-image: url({item.tournament.settings
          ?.tournamentImage ?? defaultLogo}); background-size: contain"
      />
      <Text>
        <PrimaryText>
